...
export async function recognizeContract(
  ...
  console.log(`\n========== OCR CACHE PAGE ${i + 1} ==========`);
  console.log(`Строк в кэше: ${cachedRows.length}`);

  cachedRows.slice(0, 100).forEach((row, index) => {
    console.log(`${index + 1}: ${row}`);
  });

  console.log(`========== END OCR CACHE PAGE ${i + 1} ==========\n`);
  ...
  return cachedRows;
    ...
    console.log(`\n========== OCR PAGE ${i + 1} ==========`);
console.log(`Строк распознано: ${best.rows.length}`);
console.log(`Оценка OCR: ${best.score}`);

best.rows.slice(0, 100).forEach((row, index) => {
  console.log(`${index + 1}: ${row}`);
});

console.log(`========== END OCR PAGE ${i + 1} ==========\n`);
