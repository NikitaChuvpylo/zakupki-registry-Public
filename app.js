...
function parseSpecification(pageTexts) {
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    // попытка склейки с следующей строкой той же страницы — НО только если в
    // текущей строке уже есть хотя бы одно число (значит, это действительно
    // начало «разъехавшейся» табличной строки, а не случайный кусок текста
    // без чисел вроде «Заказчик: …», который иначе мог бы по ошибке
    // «проглотить» следующую настоящую позицию целиком).
    if (!triple && decs.length >= 1 && li + 1 < lines.length && lines[li + 1].pi === pi) {
