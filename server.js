...
// --- multer: приём PDF или одной/нескольких фотографий (по странице на фото) ---
const IMAGE_RE = /\.(jpe?g|png|webp)$/i;
function isPdfFile(f) {
  return /\.pdf$/i.test(f.originalname || '') || f.mimetype === 'application/pdf';
}
function isImageFile(f) {
  return IMAGE_RE.test(f.originalname || '') || /^image\//.test(f.mimetype || '');
}
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const ok = isPdfFile(file) || isImageFile(file);
    cb(ok ? null : new Error('Только PDF или изображения (JPG/PNG/WEBP)'), ok);
  },
});
...
// Создать контракт из PDF, который уже есть в памяти как Buffer (используется
// при импорте из почты — файл туда ещё не сохранён, в отличие от /api/upload,
// где это уже сделал multer).
async function importPdfBuffer(buf, originalFilename) {
  const filename = crypto.randomUUID() + (path.extname(originalFilename || '') || '.pdf');
  ...
  try {
    await pdfToImages(pdfPath, pagesDir, 3.5);
  } catch (e) {
    console.error('Render error for contract (mail import)', id, e);
  }
  return id;
}

// --- API ---
// Загрузить PDF ИЛИ одну/несколько фотографий страниц -> создать контракт, подготовить страницы
app.post('/api/upload', upload.array('files', 40), async (req, res) => {
  try {
    ...
    const onlyPdf = files.length === 1 && isPdfFile(files[0]);
    const onlyImages = files.every(isImageFile);
    if (!onlyPdf && !onlyImages) {
      return res.status(400).json({ error: 'Загрузите один PDF-файл ИЛИ одну/несколько фотографий (JPG/PNG/WEBP), не смешивая типы' });
    }

    const ins = db.prepare(
      'INSERT INTO contracts (num, source_pdf, created_at) VALUES (?, ?, datetime(\'now\'))'
    ).run(null, onlyPdf ? files[0].filename : null);
    const id = Number(ins.lastInsertRowid);
    const pagesDir = path.join(CONTRACTS_DIR, String(id), 'pages');
    let pages = [];

    if (onlyPdf) {
      const pdfPath = path.join(UPLOADS_DIR, files[0].filename);
      ...
      // фото: каждая картинка — отдельная страница, в порядке загрузки
      fs.mkdirSync(pagesDir, { recursive: true });
      files.forEach((f, i) => {
        const ext = path.extname(f.originalname || '') || '.jpg';
        const dest = path.join(pagesDir, `page_${String(i + 1).padStart(3, '0')}${ext}`);
        fs.copyFileSync(path.join(UPLOADS_DIR, f.filename), dest);
        pages.push(dest);
      });
    }
...
// Забрать новые контракты (PDF-вложения) из почты по IMAP.
// Данные для входа — ТОЛЬКО из переменных окружения (Railway → Variables),
// никогда не передаются через фронтенд/чат.
app.post('/api/mail/fetch', async (req, res) => {
  try {
    const { fetchContractPdfsFromMail } = await import('./lib/mail.mjs');
    const seen = db.prepare('SELECT message_id FROM mail_seen WHERE message_id = ?');
    const markSeen = db.prepare('INSERT OR IGNORE INTO mail_seen (message_id, contract_id, subject) VALUES (?, ?, ?)');
    const importedContracts = [];

    const stats = await fetchContractPdfsFromMail(
      async ({ buffer, filename, messageId, subject }) => {
        try {
          const contractId = await importPdfBuffer(buffer, filename);
          markSeen.run(messageId, contractId, subject || '');
          importedContracts.push({ id: contractId, subject, filename });
        } catch (e) {
          console.error('Ошибка импорта письма', messageId, e);
          // всё равно помечаем письмо как виденное, чтобы не пытаться бесконечно на сломанном вложении
          markSeen.run(messageId, null, subject || '');
        }
      },
      (messageId) => !!seen.get(messageId)
    );

    res.json({ ...stats, imported: importedContracts });
  } catch (e) {
    console.error('Ошибка забора почты', e);
    res.status(500).json({ error: e.message });
  }
});
...
app.get('/api/contracts/:id', (req, res) => {
  ...
  const pages = fs.existsSync(pagesDir)
    ? fs.readdirSync(pagesDir).filter(f => /\.(png|jpe?g|webp)$/i.test(f)).sort().map(f => `/files/${c.id}/pages/${f}`)
    : [];
...
app.post('/api/contracts/:id/recognize', async (req, res) => {
  ...
  // если контракт загружен как PDF — передаём путь к нему, чтобы можно было
  // достать текстовый слой напрямую (без OCR) там, где это не скан
  const pdfPath = c.source_pdf ? path.join(UPLOADS_DIR, c.source_pdf) : null;
  res.json({ status: 'started' });
  // фоновая задача (не блокируем ответ)
  setImmediate(async () => {
    try {
      const job = await rec.recognizeContract(id, pagesDir, ocrCacheDir, null, pdfPath);
      if (job.status === 'done') {
